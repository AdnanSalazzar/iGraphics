...
//Sound 
bool musicShoot;
bool bcsoundLevel1 = true; //////////////////////////////////////////////////////////// MUSIC OFFED
...
void healthShow()
{
	if (healthNo < 0)
	{
		...
		*/
		...
		/*
		FILE *fp; 
		fp = fopen("highscorex.txt", "a+");
		fprintf(fp, "%s %d ", name, (scoreIndex) * 10);
		fclose(fp);
		*/
...
void iKeyboard(unsigned char key)
{
	...
	if (key == 'p' && flag_OpenScreen == 2 && key == 'P')
	{
		PlaySound("picture\\soundShoot.wav", NULL, SND_ASYNC);
...
int main()
{
	...
	setAll();
	iInitialize(screenWidth, screenHeight, "Final");
	imageLoaded();
